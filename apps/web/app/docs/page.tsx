    ...
    <main className="mx-auto max-w-[1180px]">
      <header className="mb-8 border-b border-line pb-6">
        ...
        <p className="mt-2.5 max-w-[62ch] text-sm text-muted">
          <code className="font-mono text-txt">palette-forge</code> is the
          engine behind this site, published as a zero-dependency npm package.
          It runs in the browser, in Node, in a Worker, and from the command
          line.
        </p>
      </header>
      ...
      <div className="lg:grid lg:grid-cols-[196px_minmax(0,1fr)] lg:items-start lg:gap-14">
        ...
        <article className="max-w-[76ch] text-sm leading-relaxed text-muted">
          ...
          <p>
            You have a picture. It has colours in it. You want those colours as
            code you can use in your own site or app. That is the entire tool.
          </p>
          <p className="mt-3">
            ...
            and drag a picture onto it. The colours appear. Click one to copy
            it. You&rsquo;re done. You can stop reading here.
          </p>

          <h3 className="mt-6 mb-2 font-medium text-txt">
            What the four sections mean
          </h3>
          <p>
            <b className="text-txt">The colours.</b> Each rectangle is one
            colour found in your picture, with its code (like{" "}
            <code className="font-mono text-txt">#4CC9F0</code>) and how much of
            the picture it covers.
          </p>
          <p className="mt-2">
            <b className="text-txt">The labels.</b> The tool guesses what each
            colour is <i>for</i>,{" "}
            <code className="font-mono text-txt">primary</code> is your main
            brand colour, <code className="font-mono text-txt">ink</code> is for
            text, <code className="font-mono text-txt">paper</code> is for
            backgrounds. It&rsquo;s a guess. Use the dropdown under any colour
            to correct it.
          </p>
          <p className="mt-2">
            <b className="text-txt">The contrast matrix.</b> Some colour pairs
            are hard to read, light grey text on white, for instance. This
            checks every pair and says <b className="text-ok">PASS</b> or{" "}
            <b className="text-bad">FAIL</b>. You don&rsquo;t need to understand
            the numbers. Look for PASS.
          </p>
          <p className="mt-2">
            <b className="text-txt">The generated tokens.</b> The actual code,
            ready to copy. The tabs are different flavours of the same thing,
            pick <code className="font-mono text-txt">tokens.css</code> if
            you&rsquo;re unsure, or{" "}
            <code className="font-mono text-txt">shadcn/ui</code> if you use
            shadcn (it does the most work for you).
          </p>
          ...
          <p>
            <b className="text-txt">Only got white and grey?</b> Your picture is
            probably a screenshot, and screenshots are mostly background. Tick{" "}
            <b className="text-txt">Ignore background greys</b>.
          </p>
          <p className="mt-2">
            <b className="text-txt">Too many similar colours?</b> Turn the{" "}
            <b className="text-txt">Colours</b> slider down. You&rsquo;re asking
            for more colours than the picture really has.
          </p>
          <p className="mt-2">
            <b className="text-txt">It missed a colour you can see?</b> Turn the
            slider up, or press{" "}
            <b className="text-txt">Try a different split</b>.
          </p>
...
# or: pnpm add palette-forge · yarn add palette-forge · bun add palette-forge`}</Code>
          <p className="mt-3">
            No runtime dependencies in the browser path. PNG and JPEG decoding
            in Node is bundled;{" "}
            <code className="font-mono text-txt">sharp</code> is an optional
            peer dependency that unlocks WebP, AVIF, TIFF, GIF and HEIC.
          </p>

          <H2 id="browser">In the browser</H2>
          <p>
            <code className="font-mono text-txt">extractPaletteFromImage</code>{" "}
            accepts a <code className="font-mono text-txt">File</code>,{" "}
            <code className="font-mono text-txt">Blob</code>,{" "}
            <code className="font-mono text-txt">&lt;img&gt;</code>,{" "}
            <code className="font-mono text-txt">ImageBitmap</code>, canvas or
            URL.
          </p>
...
console.log(toCSS(palette));`}</Code>
          <p className="mt-3">
            Extraction is deterministic: the same image and options always
            produce the same palette, so results can be cached, diffed and
            asserted against in tests.
          </p>

          <H2 id="react">React hooks</H2>
          <p>
            <code className="font-mono text-txt">palette-forge/react</code> is
            headless, hooks and state, no markup and no styles.
          </p>
          <Code>{`"use client";
    ...
    <div {...rootProps} data-over={isOver}>
      ...
      {palette?.swatches.map((s) => (
        <span key={s.hex} style={{ background: s.hex, color: s.on }}>
          ...
          <p className="mt-3">
            The decoded pixels are cached separately from the palette, so
            changing <code className="font-mono text-txt">colors</code>{" "}
            re-clusters in a millisecond or two without re-decoding the image,
            which is what makes a live slider feel instant.
          </p>
...
await writeFile("app/globals.css", toShadcn(palette));`}</Code>
          <p className="mt-3">
            Also available:{" "}
            <code className="font-mono text-txt">extractPaletteFromBuffer</code>
            , <code className="font-mono text-txt">extractPaletteFromUrl</code>{" "}
            and <code className="font-mono text-txt">decodeImage</code>.
          </p>
...
emit(palette, "css", { prefix: "brand" });     // --brand-primary: …`}</Code>
          <p className="mt-3">
            The <code className="font-mono text-txt">shadcn</code> emitter is
            the shortcut worth knowing: it derives a full semantic theme,
            background, card, popover, muted, border, ring, destructive, for
            light and dark, and contrast-repairs every text/surface pair to AA
            before emitting.
          </p>
...
ensureContrast("#4cc9f0", "#ffffff", { target: 7 });  // → "#00617a" (7.02:1)`}</Code>
          ...
          <p>
            Ramps are generated in OKLCH and gamut-mapped by reducing chroma, so
            hue holds across the whole ramp instead of skewing at the ends.
          </p>
...
harmony.complementary("#4cc9f0");          // OKLCH hue rotation`}</Code>
          <p className="mt-3">
            By default the source colour is anchored: it appears verbatim at its
            nearest stop, so the brand colour is genuinely in the ramp rather
            than approximated.
          </p>
          ...
          <p>
            ...
            <code className="font-mono text-txt">POST /api/extract</code> for
            consumers without a canvas, CI jobs, bots, plugins.
          </p>
          <Code lang="bash">{`curl -X POST https://palette-forge-web.vercel.app/api/extract \\
...
curl -X POST https://palette-forge-web.vercel.app/api/extract \\
  ...
  -d '{"url":"https://example.com/logo.png","format":"shadcn"}'`}</Code>
          <p className="mt-3">
            The browser app never calls it, extraction there is entirely local,
            and no image ever leaves the machine.
          </p>
          ...
          <dl className="grid gap-x-6 gap-y-3 sm:grid-cols-[auto_1fr]">
            {(
              ...
              <div key={term} className="contents">
                <dt className="font-mono text-[12px] text-txt sm:text-right">
                  {term}
                </dt>
